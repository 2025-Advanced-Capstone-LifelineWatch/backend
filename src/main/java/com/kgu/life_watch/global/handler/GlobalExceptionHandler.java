 ...
 * 전역 예외 처리 클래스
 *
 * 컨트롤러(@RestController)에서 발생하는 예외를 일괄적으로 처리하여
 * 클라이언트에게 일관된 JSON 응답(ApiResponse) 형태로 반환합니다.
 *
 * - LifelineException: 사용자 정의 예외를 처리하고 적절한 HTTP 상태 코드와 메시지를 반환
 * - MethodArgumentNotValidException: @Valid 유효성 검증 실패 시 발생하는 예외를 처리
 *
 * 이 클래스는 Spring의 ResponseEntityExceptionHandler를 상속받아,
 * Spring MVC 기본 예외 처리 로직도 오버라이딩하여 커스터마이징할 수 있습니다.
 */
...
public class GlobalExceptionHandler extends ResponseEntityExceptionHandler {
     ...
     *
     * 사용자 정의 예외에 포함된 ErrorCode를 추출하여,
     * 해당 상태 코드와 메시지를 담은 ApiResponse를 JSON 형식으로 반환합니다.
     */
     ...
     *
     * 발생한 유효성 오류 중 첫 번째 메시지를 기반으로 ErrorCode를 매핑한 후,
     * 클라이언트에게 ApiResponse 형태로 오류 정보를 반환합니다.
     *
     * 참고: HTTP 상태 코드는 항상 200 OK로 반환되며,
     * 실제 에러 여부는 응답 본문의 status 필드(ErrorCode)로 판단합니다.
     */
    @Override
